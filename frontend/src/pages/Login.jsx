...
const Login = () => {
  ...
  const sbmitHandler = async (event) => {
  ...
  return (
    <form className="min-h-[80vh] flex items-center ">
      <div className="flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-zinc-600 text-sm shadow-lg">
        ...
        {
          state === 'Sign Up' ? <p>Already have an account?<span className="text-primary cursor-pointer underline" onClick={() => setstate('Log In')}> Log in</span>
          </p> : <p>Don&apos;t have an account? <span className="text-primary cursor-pointer underline" onClick={() => setstate('Sign Up')}>Sign up</span></p>
        }
      </div>
